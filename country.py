import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import plotly.express as px
import plotly.graph_objects as go

# Set page config for better layout
st.set_page_config(page_title="RHoMIS Dataset Explorer", layout="wide")

# Load the dataset (assuming the file is in the same directory or provide the path)
@st.cache_data
def load_data():
    df = pd.read_csv("/home/jakes/Documents/strathmore/Modules/Module 1/dataMining/project/Rhomis/final/RHoMIS_Indicators.csv", encoding='latin1')
    # Basic data cleaning: Convert numeric columns if needed, handle NA
    numeric_cols = [
        'HHsizemembers', 'HHsizeMAE', 'LandCultivated', 'LivestockHoldings',
        'NrofMonthsFoodInsecure', 'PPI_Likelihood', 'score_HDDS_GoodSeason',
        'score_HDDS_BadSeason', 'total_income_USD_PPP_pHH_Yr', 'offfarm_income_USD_PPP_pHH_Yr',
        'farm_income_USD_PPP_pHH_Yr', 'Food_Availability_kCal_MAE_day',
        'GHGEmissions', 'NFertInput'
    ]
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    return df

df = load_data()

# -------------------------------
# Sidebar Filters
# -------------------------------
st.sidebar.header("Filters")

countries = sorted(df['ID_COUNTRY'].dropna().unique())
selected_countries = st.sidebar.multiselect("Countries", countries, default=countries[:1])

years = sorted(df['YEAR'].dropna().unique())
selected_years = st.sidebar.multiselect("Years", years, default=years)

# Filter
filtered_df = df[
    df['ID_COUNTRY'].isin(selected_countries) &
    df['YEAR'].isin(selected_years)
].copy()

if filtered_df.empty:
    st.warning("No data matches the selected filters.")
    st.stop()

# -------------------------------
# Tabs
# -------------------------------
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Overview", "Summary Stats", "Distributions", "Relationships", "Comparisons & Map"
])

# -------------------------------
# Tab 1: Overview
# -------------------------------
with tab1:
    st.header("Data Overview")
    col1, col2 = st.columns([1, 3])
    with col1:
        st.metric("Households", len(filtered_df))
        st.metric("Countries", len(selected_countries))
    with col2:
        st.write(f"**Selected:** {', '.join(selected_countries)} | "
                 f"**Years:** {', '.join(map(str, selected_years))}")

    with st.expander("Sample Data"):
        st.dataframe(filtered_df.head(10))

# -------------------------------
# Tab 2: Summary Stats
# -------------------------------
with tab2:
    st.header("Summary Statistics")

    numeric_features = [
        'HHsizemembers', 'HHsizeMAE', 'LandCultivated', 'LivestockHoldings',
        'NrofMonthsFoodInsecure', 'score_HDDS_GoodSeason', 'total_income_USD_PPP_pHH_Yr',
        'GHGEmissions', 'NFertInput'
    ]
    cat_features = ['HouseholdType', 'Head_EducationLevel', 'HFIAS_status']

    sel_num = st.multiselect("Numeric Features", numeric_features, default=numeric_features[:3])
    if sel_num:
        st.subheader("Numeric Summary")
        st.dataframe(filtered_df[sel_num].describe())

    sel_cat = st.multiselect("Categorical Features", cat_features, default=cat_features[:2])
    for cat in sel_cat:
        with st.expander(f"{cat} – Value Counts"):
            st.dataframe(filtered_df[cat].value_counts().head(10))

# -------------------------------
# Tab 3: Distributions
# -------------------------------
with tab3:
    st.header("Feature Distributions")

    dist_num = st.multiselect(
        "Select Numeric Features", numeric_features, default=numeric_features[:3], key="dist_num"
    )
    for feat in dist_num:
        with st.expander(f"Distribution of **{feat}**"):
            fig_hist = px.histogram(
                filtered_df, x=feat, color='ID_COUNTRY',
                marginal="box", title=f"{feat} – Histogram + Box",
                hover_data=['ID_HH']
            )
            st.plotly_chart(fig_hist, use_container_width=True)

            fig_box = px.box(filtered_df, x='ID_COUNTRY', y=feat, points="outliers")
            st.plotly_chart(fig_box, use_container_width=True)

# -------------------------------
# Tab 4: Relationships
# -------------------------------
with tab4:
    st.header("Relationships & Correlations")

    rel_num = st.multiselect(
        "Select Features for Correlation", numeric_features, default=numeric_features[:5], key="rel_num"
    )
    if len(rel_num) > 1:
        with st.expander("Correlation Heatmap"):
            corr = filtered_df[rel_num].corr()
            fig = px.imshow(corr, color_continuous_scale='RdBu_r', aspect="auto")
            st.plotly_chart(fig, use_container_width=True)

    if len(rel_num) >= 2:
        x_feat = st.selectbox("X-axis", rel_num, index=0)
        y_feat = st.selectbox("Y-axis", rel_num, index=1)
        with st.expander(f"Scatter: {x_feat} vs {y_feat}"):
            fig = px.scatter(
                filtered_df, x=x_feat, y=y_feat,
                color='ID_COUNTRY', size='HHsizemembers',
                hover_data=['ID_HH', 'HFIAS_status'],
                trendline="ols" if len(selected_countries) == 1 else None
            )
            st.plotly_chart(fig, use_container_width=True)

# -------------------------------
# Tab 5: Comparisons & Map
# -------------------------------
with tab5:
    st.header("Country Comparisons & Geography")

    if len(selected_countries) > 1:
        with st.expander("Average Values by Country"):
            avg_cols = st.multiselect("Features to Compare", numeric_features, default=numeric_features[:3], key="avg_cols")
            if avg_cols:
                avg_df = filtered_df.groupby('ID_COUNTRY')[avg_cols].mean().reset_index()
                fig = px.bar(
                    avg_df.melt(id_vars='ID_COUNTRY'), x='ID_COUNTRY', y='value',
                    color='variable', barmode='group', title="Average Feature Values"
                )
                st.plotly_chart(fig, use_container_width=True)

    # --- Interactive Map (Safe) ---
    map_df = filtered_df.dropna(subset=['GPS_LAT', 'GPS_LON', 'LandCultivated'])
    if not map_df.empty:
        with st.expander("Geographic Distribution"):
            # Scale size safely
            size_vals = map_df['LandCultivated'].clip(lower=0.1) * 5  # scale factor

            fig_map = px.scatter_mapbox(
                map_df,
                lat='GPS_LAT', lon='GPS_LON',
                size=size_vals,
                color='ID_COUNTRY',
                hover_data=['ID_HH', 'HFIAS_status', 'LandCultivated'],
                mapbox_style="open-street-map",
                zoom=5,
                title="Household Locations (Size = Land Cultivated)"
            )
            fig_map.update_layout(margin=dict(l=0, r=0, t=40, b=0))
            st.plotly_chart(fig_map, use_container_width=True)
    else:
        st.info("No valid GPS + LandCultivated data for mapping.")

# -------------------------------
# Download
# -------------------------------
st.markdown("---")
st.header("Download Filtered Data")
csv = filtered_df.to_csv(index=False).encode('utf-8')
st.download_button(
    label="Download CSV",
    data=csv,
    file_name=f"rhomis_{'_'.join(selected_countries)}_{'_'.join(map(str, selected_years))}.csv",
    mime="text/csv"
)